...
if __name__ == "__main__":
    ...
    pad = 3
    padded = np.zeros(
        (downscaled.shape[0], downscaled.shape[1] + 2 * pad, downscaled.shape[2] + 2 * pad, downscaled.shape[3]))
    padded_test = np.zeros((downscaled_test.shape[0], downscaled_test.shape[1] + 2 * pad,
    ...
    history = model.fit(padded, HDimages, verbose=2, epochs=5, validation_split=.2)

    result = model.predict(padded_test)
    print("Upscaled from (%d, %d) to (%d, %d)" % (
    downscaled.shape[1], downscaled.shape[2], result.shape[1], result.shape[2]))
